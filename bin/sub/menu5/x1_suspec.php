					...
					<fieldset>
						...
						<div class="row">
							...
							<p class="response_block" style="margin-left:10px">
								...
								<span class="btn_wrap">
									<a href="#" class="btn_s white hover_btn btn_search " style="width:140px;margin: 0px;" onclick="common_ser();">사원별조회</a>
									<a class="btn_s white hover_btn btn_search2 excelBtn" style="width:100px;margin: 0px;">엑셀</a>
								</span>
							</p>
						</div>
					</fieldset>
...
function suspecPopOpen(skey,yymm){

	var left = Math.ceil((window.screen.width - 1250)/2);
	var top = Math.ceil((window.screen.height - 880)/2);
	var popOpen	= window.open("<?=$conf['homeDir']?>/sub/menu5/x1_suspec_list_detail.php?SKEY="+skey+"&yymm="+yymm,"swon","width=1250px,height=800px,top="+top+",left="+left+",status=0,toolbar=0,menubar=0,location=false,scrollbars=yes");
}
...
function common_ser(){
	...
	// ajax 폼데이터 전송
	ajaxLodingForm('x1_suspec_list.php',$('#suspec'),formData);
	*/
	...
	ajaxLodingTarket('x1_suspec_list.php',$('#suspec'),'&SDATE1='+sdate1+'&SDATE2='+sdate2+'&id='+id+'&page='+page);
...
$(document).ready(function(){	
	...
	$(".excelBtn").click(function(){
		if($('#excelcnt').val() == 0 ){
			...
			if(confirm("엑셀로 내려받으시겠습니까?")){
				//$("#div_load_image").show();
				$("form[name='searchFrm']").attr("action","x1_suspec_excel.php");
	...
	//window.parent.postMessage("수수료관리 > 설계사별급여명세서", "*");   // '*' on any domain 부모로 보내기..        
