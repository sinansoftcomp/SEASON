...
//--> union으로 한이유는 비매칭칭 사원의 경우 기준테이블이 존재하지 않아 data가 누락되기 때문이다.
  $sql= "
	...
	from(
			...
			select     a.*,  e.BNAME,f.JSNAME,g.JNAME,h.TNAME,  j.NAME , row_number()over(order by  a.BONBU,a.JISA,a.JIJUM,a.TEAM ,a.INSCODE) rnum from   	
					(SELECT a.SCODE, isnull(c.BONBU,'') BONBU ,isnull(c.JISA,'') JISA ,isnull(c.JIJUM,'') JIJUM ,isnull(c.TEAM,'') TEAM  ,isnull(a.INSCODE,'') INSCODE,
					...
					group by   a.scode,isnull(c.BONBU,''),isnull(c.JISA,''),isnull(c.JIJUM,''),isnull(c.TEAM,'') ,isnull(a.INSCODE,'')  ) a 
...
<div class="tb_type01 kwndatalist div_grid2 rowspan" style="overflow-y:auto;">	
	<table id="sort_table_swonlist" class="gridhover" style="min-width:1600px; "  >
		...
		<thead>
			<tr class="rowTop">
				<th rowspan=2 align="center" >소속</th>	
				<th rowspan=2 align="center" style=" border-right: 1px solid #c7c7c7;">원수사</th>			
		...
		<tbody>
			...
			<tr class="rowData" rol-data='<?=$swonskey?>', rol-yymm='<?=$yymm?>'>
				...
				<td align="left" style=" border-right: 1px solid #c7c7c7;"><?=$NAME?></td>
