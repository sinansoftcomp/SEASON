...
//--> union으로 한이유는 비매칭칭 사원의 경우 기준테이블이 존재하지 않아 data가 누락되기 때문이다.
 $sql= "
	...
	from(
				...
				select  a.*, c.SNAME,c.POS,e.BNAME,f.JSNAME,g.JNAME,h.TNAME, i.SUBNM, row_number()over(order by c.BONBU,c.JISA,c.JIJUM,c.TEAM,c.SNAME) rnum from 
...
<div class="tb_type01 kwndatalist div_grid2 rowspan" style="overflow-y:auto;">	
	<table id="sort_table_swonlist" class="gridhover" style="min-width:1530px; "  >
		<colgroup>
