...
//--> union으로 한이유는 비매칭칭 사원의 경우 기준테이블이 존재하지 않아 data가 누락되기 때문이다.
  $sql= "
	...
	from(
			...
			select     a.*,  e.BNAME,f.JSNAME,g.JNAME,h.TNAME,  j.NAME , row_number()over(order by   a.INSCODE ,a.BONBU,a.JISA,a.JIJUM,a.TEAM) rnum from   	
					...
					group by   isnull(a.INSCODE,'') , a.scode,isnull(c.BONBU,''),isnull(c.JISA,''),isnull(c.JIJUM,''),isnull(c.TEAM,'')  ) a 
