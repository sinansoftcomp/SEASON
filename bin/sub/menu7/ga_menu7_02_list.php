...
if($_REQUEST['id']){
	...
	if($Ngubun == 'N1'){
		$where  .= "" ;
		if($_REQUEST['skey']){
			$where  .= " where c.skey = '".$_REQUEST['skey']."' " ;
		}
	}else {
		$inscode= substr($_REQUEST['id'],2,10);
		$where  .= " where    a.INSCODE = '".$inscode."' " ;
		if($_REQUEST['skey']){
			$where  .= " and c.skey = '".$_REQUEST['skey']."' " ;
		}
	}
}
...
//--> union으로 한이유는 비매칭칭 사원의 경우 기준테이블이 존재하지 않아 data가 누락되기 때문이다.
 $sql= "
	...
	from(
				...
				select  a.*, j.NAME, c.SNAME,c.POS,e.BNAME,f.JSNAME,g.JNAME,h.TNAME, i.SUBNM, row_number()over(order by j.INSCODE, c.BONBU,c.JISA,c.JIJUM,c.TEAM,c.SNAME) rnum from 
						(SELECT SCODE, INSCODE, SKEY ,sum(IMST1) IPMST1,sum(IMST2) IPMST2,sum(IMST3) IPMST3,sum(IMST4) IPMST4 ,SUM(SU1) SU1,SUM(SU2) SU2,SUM(SU3) SU3,SUM(SU4) SU4,   sum(IMST4) -  SUM(SU4) CATOT , SUM(SUNAB) SUNAB  ,
							...
							left outer join inssetup(nolock) j  on a.scode = j.scode and a.INSCODE = j.INSCODE	 
							$where  
	...
	<table id="sort_table_swonlist" class="gridhover" style="min-width:1600px; "  >
		...
		<thead>
			<tr class="rowTop">
				...
				<th rowspan=2 align="center">직급</th>
