...
$tree_arr['id'] ="N10000";            
$tree_arr['parent'] =  "#";  
$tree_arr['text'] =  iconv("EUCKR","UTF-8","GA직급정보");  
$tree_arr['icon'] =  "glyphicon fa-solid fa-building-user jisa";  
array_push($tree_arr_tot,$tree_arr) ;	

//-->직위 
$sql = "SELECT *  FROM COMMON where    scode = '".$_SESSION['S_SCODE']."'  and  code = 'COM006' order by num  ";
$res = sqlsrv_query( $mscon, $sql );
	while( $row = sqlsrv_fetch_array($res) ) { 
		unset($tree_arr); 
		$tree_arr['id'] = 'N2'.$row["CODESUB"];            
		$tree_arr['parent'] =  "N10000";  
		$tree_arr['text'] =  iconv("EUCKR","UTF-8",$row["SUBNM"]);  
