...
$gubun = "A"; // 사원원수사코드(구코드)컨버전
$gubunname = "사원원수사코드(구코드)컨버전";
...
<div class="container">
	<div class="content_wrap">
		<fieldset>
			<legend>사원원수사코드(구코드)컨버전엑셀업로드</legend>
			<h2 class="tit_big">사원원수사코드(구코드)컨버전엑셀업로드</h2>

			<!-- 검색조건 -->
			<div class="tit_wrap mt20">
				<span>	
					시트자체 사원원수사코드(구코드)컨버전번호가 중복되면 수정건수와 디테일 건수가 중복된 DATA만큼 오차발생함
				</span>
				<span class="btn_wrap">
   			 	    <a href="#" class="btn_l white" style="min-width:100px;" onclick="upload();">사원원수사코드(구코드)컨버전엑셀업로드</a>
				</span>	  
...
// 상품관리 모달창
function upload(){
	$.ajaxLoding('ga_swon3_01_upload.php',$('.layerBody_item'),$('#modal2'));	
}
 ...
 $(document).ready(function(){
	...
	$(".rowData > td").click(function(){
		...
		if($(trData).find("td").index($(this))!='10'){
			var popOpen	= window.open("<?=$conf['homeDir']?>/sub/menu3/ga_swon3_01_upload.php?upldate="+upldate+"&gubun=" +gubun+"&gubunsub=" + gubunsub+"&uplnum=" +uplnum +"&filename=" +filename                    ,"width=1200px,height=950px,top="+top+",left="+left+",status=0,toolbar=0,menubar=0,location=false,scrollbars=yes");
			popOpen.focus();
		} 
	});
