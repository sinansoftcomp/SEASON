...
require_once $_SERVER['DOCUMENT_ROOT']."/bin/include/lib/PHPExcel/Classes/PHPExcel/IOFactory.php";  // IOFactory.php을 불러와야 하며, 경로는 사용자의 설정에 맞게 수정해야 한다.					
	...
	function pattern_ser ($objWorksheet,$mscon,$col_cnt){
		...
		$ser_tit_yes  = 'N';		
		for ($i = 1 ; $i <= 20 ; $i++) {		//첫행부터 35행 검사 (타이틀 라인찿기)
		...
		$sql = "select *  from UPLOAD_EXCEL where scode = '".$_SESSION['S_SCODE']."' and gubun = 'A'   order by upcnt desc";
		//$sql = "select *  from UPLOAD_EXCEL where scode = '".$_SESSION['S_SCODE']."' and gubun = 'A'  and gubunsub ='P2' ";
...
//				for ($i = 1 ; $i <= $col_cnt ; $i++) {		//2열부터 데이터시작
				for ($i = 1 ; $i <=303 ; $i++) {		//2열부터 데이터시작
					...
					$C = "C".$ex_seq; 
					 ...
					 if ($ex_i  == 2) {
						 $EXCEL_declare[$ex_seq ][2] = str_replace(" ","", $ptData[$db_i][$B]);
