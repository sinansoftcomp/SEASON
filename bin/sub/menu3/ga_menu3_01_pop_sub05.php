	...
	$sql= "
		...
		from(
			select  a.id,
					...
					a.tdate,
					a.item,
					a.itemnm full_itemnm,
					dbo.GetCutStr(a.itemnm,20,'..') itemnm,
					a.mamt,
					a.hamt,
					a.samt,
					a.kstbit,
					...
					a.uplseq,
					a.oridata,
					g.filename full_filename,
					dbo.GetCutStr(g.filename,20,'..') filename,
					row_number()over(order by a.id desc) rnum
			from kwn_his(nolock) a	
...
<div class="tb_type01" style="height:650px;overflow-y:auto;border-top: 1px solid #47474a;">
	<table id="sort_sub05" class="gridhover">
		<colgroup>
			<col width="75px">
			<col width="120px">
			<col width="75px">
		...
		<tr class="rowTop">
			...
			<th align="right">계약개시일자</th>
			<th align="right">계약종료일자</th>
		...
		<tbody>
			...
			<tr class="rowData" rol-data='<?=$oridata?>' rol-file='<?=$filename?>' rol-iseq='<?=$uplseq?> '>
				<td><?if(trim($upldate)) echo date("Y-m-d",strtotime($upldate))?></td>
				<td align="left" title="<?=$full_filename?>"><?=$filename?></td>
				...
				<td align="left" title="<?=$full_itemnm?>"><?=$itemnm?></td>
				<td align="right"><?=number_format($mamt)?></td>
				<td align="right"><?=number_format($hamt)?></td>
...
<div style="text-align: center">		
	<ul class="pagination pagination-sm pop_sub05" style="margin: 10px">
