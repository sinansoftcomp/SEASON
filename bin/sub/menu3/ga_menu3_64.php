					...
					<fieldset>
						...
						<div>
							...
							<p class="response_block" style="margin-left:10px">
								...
								<span class="btn_wrap">
									<a class="btn_s white hover_btn btn_search" style="width:150px;margin: 0px;" onclick="common_ser();">수금조회</a>
									<a class="btn_s white" id="btn_ins" style="width:150px;" onclick="common_ser_notser('');">수금비매칭사용인조회</a>													
								</span>
							</p>
						</div>
					</fieldset>
...
function common_ser(){
	...
	// ajax 폼데이터 전송
	ajaxLodingForm('ga_menu3_64_list.php',$('#kwnlist'),formData);
}
...
function common_ser_notser(){
	$("#div_load_image").show();
	var formData = $("#searchFrm").serialize();
	ajaxLodingForm('ga_menu3_64_list_2.php',$('#kwnlist'),formData);
}
...
$(document).ready(function(){	
	...
	// 계약상세정보 리스트
	ajaxLodingTarket('ga_menu3_64_list.php',$('#kwnlist'),'');

	// 조직도 호출(/bin/include/source/bottom 에 존재)
	get_jstree();

	//window.parent.postMessage("계약수납관리 > 수금명세서", "*");   // '*' on any domain 부모로 보내기..        
