					...
					<fieldset>
						...
						<div class="row">
							...
							<p class="response_block" style="margin-left:10px">
								...
								<div style="float:right"><a class="btn_s white btn_off"	name="eventgubun"   id="eventgubun" onclick="event_change();">이동</a></div>
							</p>
							...
							<span class="btn_wrap" >
								...
								<a class="btn_s white btn_off" style="width:113px;" onclick="smspop();">SMS전송</a>
								<a class="btn_s white btn_off excelBtn" style="width:113px;">엑셀</a>
							</span>
...
function common_ser(){
	...
	// ajax 폼데이터 전송
	ajaxLodingForm('ga_menu3_01_list.php',$('#kwnlist'),formData);
...
function event_change(){
	...
	if($("#eventgubun").text() == '정렬'){
		$("#eventgubun").text('이동');
		common_ser();
	}else if($("#eventgubun").text() == '이동'){
		$("#eventgubun").text('정렬');
		$('table').removeClass('table_move');
	}
...
$(document).ready(function(){	
	...
	$(".excelBtn").click(function(){
		if($('#excelcnt').val() == 0 ){
			...
			if(confirm("엑셀로 내려받으시겠습니까?")){
				//$("#div_load_image").show();
				$("form[name='searchFrm']").attr("action","ga_menu3_01_excel.php");
	...
	// 계약상세정보 리스트
	ajaxLodingTarket('ga_menu3_01_list.php',$('#kwnlist'),'&SDATE1=<?=$sdate1?>&SDATE2=<?=$sdate2?>');
