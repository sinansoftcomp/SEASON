...
$fyymmdd=  substr($sdate1,0,4).substr($sdate1,5,2).substr($sdate1,8,2); 
$tyymmdd = substr($sdate2,0,4).substr($sdate2,5,2).substr($sdate2,8,2); 
...
from(
	...
	from(
		select      substring(a.upldate,1,4) + '-' +substring(a.upldate,5,2) +'-'+ substring(a.upldate,7,2)  +'-'+a.gubun+'-'+a.gubunsub +'-'+ CONVERT(VARCHAR(5) , a.uplnum ) as  upno ,   a.scode,a.upldate, a.gubun,a.gubunsub,  a.uplnum, d.GNAME, a.filename, b.name, a.cnt, a.amt,
								...
								convert(varchar,a.idate,21) idate ,c.sname , isnull(a.fcnt,0) fcnt , isnull(a.famt,0) famt  ,a.code , e.samt
		from upload_history a left outer join insmaster b on a.code = b.code
								...
								left outer join 
										(SELECT  SCODE , IPDATE  , GUBUN  , GUBUNSUB  , INO ,SUM(SAMT)  samt
															FROM  INS_SUNAB 
															GROUP BY SCODE , IPDATE  , GUBUN  , GUBUNSUB , INO)	 e
											on a.scode = e.scode and  a.UPLDATE = e.IPDATE and  a.GUBUN = e.GUBUN  and a.gubunsub =e.gubunsub  and a.UPLNUM = e.INO
		where a.scode = '".$_SESSION['S_SCODE']."' and a.gubun = 'S'  and a.upldate >= '".$fyymmdd."'  and  a.upldate <= '".$tyymmdd."'
	) tbl
) p
		...
		<fieldset>
			...
			<div class="box_wrap sel_btn">
				<form name="searchFrm" id="searchFrm" method="get" action="<?$_SERVER['PHP_SELF']?>">
					...
					<fieldset>
					<div class="row">
						<span class="ser_font">업로드일시</span> 
						...
						<p class="response_block" style="margin-left:10px">
							...
							<span class="btn_wrap">			
								...
								<a class="btn_s white" style="min-width:150px;" onclick="upload();">원수사수납엑셀업로드</a>
							</span>	  
						</p>
					</div>
					</fieldset>
			...
			<div class="tb_type01 div_grid" style="overflow-y:auto;">
				<form name="excelupload_form" class="ajaxForm" method="post" action="ga_menu3_62_action.php" ENCTYPE="multipart/form-data">
...
// 업로드관리 모달창
function upload(){
	$.ajaxLoding('ga_menu3_62_upload.php',$('.layerBody_item'),$('#modal2'));	
}
...
function common_ser(){ 
 ...
 $(document).ready(function(){
	...
	//window.parent.postMessage("계약수납관리> 수금정보업로드", "*");   // '*' on any domain 부모로 보내기..        
	...
	$(".rowData > td").click(function(){
		...
		if($(trData).find("td").index($(this))!='9'){
			var popOpen	= window.open("<?=$conf['homeDir']?>/sub/menu3/ga_menu3_62_list.php?upldate="+upldate+"&gubun=" +gubun+"&gubunsub=" + gubunsub+"&uplnum=" +uplnum +"&filename=" +filename                    ,"width=1200px,height=950px,top="+top+",left="+left+",status=0,toolbar=0,menubar=0,location=false,scrollbars=yes");
			popOpen.focus();
		} 
	});
