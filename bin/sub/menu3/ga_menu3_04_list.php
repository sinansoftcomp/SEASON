...
$sql= "
	...
	from(
		select 
				...
				a.pname,
				a.kdate,
				isnull(a.fdate,'') fdate,
				a.tdate,
				case when isnull(a.tdate,'') != '' then DATEDIFF(DAY, getdate(), a.tdate) else '' end dday,				
				a.item,
				a.itemnm,
				a.mamt,
				a.hamt,
				...
				row_number()over(order by a.tdate , f.name, a.kname) rnum
		from kwn(nolock) a	
		...
		where a.scode = '".$_SESSION['S_SCODE']."'
		  and a.tdate between '".str_replace('-','',$sdate1)."' and '".str_replace('-','',$sdate2)."'  ".$where."
		) p
	...
	<table id="sort_table" class="gridhover" style="min-width: 3000px;">
		...
		<tbody>
			...
			<tr class="rowData" rol-date='<?=$kcode?>' rol-date2='<?=$inscode?>'>
				...
				<td><?if(trim($tdate)) echo date("Y-m-d",strtotime($fdate)).' ~ '.date("Y-m-d",strtotime($tdate))?></td>			
				<td align="right" style="color: red;"><?=$dday?></td>
			...
			<?}}else{?>
				<tr>
					<td style="color:#8C8C8C" colspan=25>검색된 데이터가 없습니다</td>
				</tr>
			<?}?>
		</tbody>
	</table>
...
$(document).ready(function(){
	...
	$("#where").val('<?=$wherescript?>');
	$("#cnt").val('<?=$totalResult_sms["CNT"]?>');
