...
$sql= "
	...
	from(
		select 
				a.*,  d.NAME, e.FILENAME,  a.SAMT6 + a.SAMT7 + a.SAMT8+ a.SAMT9+ a.SAMT10+ a.SAMT11+ a.SAMT12+ a.SAMT13 as  KITA_HAP ,
				a.SAMT1 +a.SAMT2 +a.SAMT3 +a.SAMT4 +a.SAMT5 +a.SAMT6 + a.SAMT7 + a.SAMT8+ a.SAMT9+ a.SAMT10+ a.SAMT11+ a.SAMT12+ a.SAMT13 as S_TOTAL ,
				row_number()over(order by a.INSCODE, a.iseq ) rnum
...
// 전체보험사
$sql= "select inscode code, name, gubun from inssetup(nolock) where scode = '".$_SESSION['S_SCODE']."' and useyn = 'Y' order by name";
$qry= sqlsrv_query( $mscon, $sql );
$instot	= array();
while( $fet = sqlsrv_fetch_array( $qry, SQLSRV_FETCH_ASSOC) ) {
  $instot[] = $fet;
...
$pagination = new Pagination(array(
		'base_url' => $_SERVER['PHP_SELF']."?swon=".$swon."&yymm=".$yymm."&ser_con=".$ser_con    ,
		'per_page' => $page_row,
	...
	<div class="content_wrap">
		<fieldset>
			...
			<div class="tit_wrap mt20">
				<span class="tit_big">사용인별 수입수수료 명세서 </span> 
				<span class="btn_wrap">				
			...
			<div class="tb_type01 div_grid3" style="overflow-y:auto;">
				<table id="sort_table" class="gridhover"  style="width: 4500px;">
					<colgroup>
						<col width="50px">
						<col width="90px">
						<col width="70px">
						...
						<col width="100px">  <!--계약상태-->
						<col width="100px">  <!--납입방법-->
						<col width="100px">  <!--납입주기-->
						<col width="100px">  <!--보험개시일-->
						<col width="100px">  <!--보험종료일	-->
						<col width="110px">  <!--원계약청약일	-->
						<col width="100px">  <!--청약일-->
						<col width="100px">  <!--영수일-->
						<col width="100px"> <!--피보험자-->
						<col width="100px">  <!--차량번호-->
						<col width="100px">
						<col width="100px">
						<col width="200px">
						<col width="100px">
					...
					<tbody>
						...
						<tr class="rowData" rol-data='<?=$ORIDATA?>' rol-file='<?=$FILENAME?>' rol-iseq='<?=$ISEQ?> '>
							...
							<td align="center"><?=mb_substr($NBIT,0,7, 'euc-kr')?></td>
							...
							<td align="center"><?=$BCON?></td>
							<td align="center"><?=$DCODE?></td>
							<td align="left"><?=mb_substr($DNAME,0,10, 'euc-kr').'..'?></td>

							<td align="center"><?=$GKCODE?></td>
							<td align="center"><?=$BKIKAN?></td>
