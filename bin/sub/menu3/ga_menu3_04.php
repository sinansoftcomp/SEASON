				...
				<input type="hidden" name="page" id="page" value="">
					<fieldset>
						...
						<div class="row">
							...
							<select name="inscode" id="inscode" class="srch_css" style="margin-left:0;width:190px">				
							...
							<select name="kstbit" id="kstbit" class="srch_css" style="width:115px;margin-left:9px"> 
							...
							<select name="searchF1" id="searchF1" class="srch_css" onchange="fn_srch(this.value);" style="width:114px;margin-left:9px">
							...
							<span class="btn_wrap" >
								...
								<a class="btn_s white hover_btn btn_off" style="width:113px;margin:0" onclick="smspop();">SMS전송</a>
							</span>
...
function smspop(){
	...
	var popOpen	= window.open("<?=$conf['homeDir']?>/sub/help/sms_pop.php?sdate1="+sdate1+"&sdate2="+sdate2+"&where="+where+"&cnt="+cnt+"&sms_type="+sms_type,"smskwn1","width=500px,height=400px,top="+top+",left="+left+",status=0,toolbar=0,menubar=0,location=false,scrollbars=yes");
	popOpen.focus();
...
function common_ser(){
	...
	// ajax 폼데이터 전송
	ajaxLodingForm('ga_menu3_04_list.php',$('#kwnlist'),formData);
...
$(document).ready(function(){	
	// 계약상세정보 리스트
	ajaxLodingTarket('ga_menu3_04_list.php',$('#kwnlist'),'&SDATE1=<?=$sdate1?>&SDATE2=<?=$sdate2?>');

	// 조직도 호출(/bin/include/source/bottom 에 존재)
	get_jstree();

	//window.parent.postMessage("계약수납관리 > 계약만기조회", "*");   // '*' on any domain 부모로 보내기..        
