	...
	<div class="content_wrap">
		<fieldset>
			...
			<div class="tit_wrap mt20">
				<span class="ser_font">	
					시트자체 계약번호가 중복되면 수정건수와 디테일 건수가 중복된 DATA만큼 오차발생함
				</span>
				<span class="btn_wrap">
   			 	    <a class="btn_s white" style="min-width:250px;" onclick="upload();">계약엑셀업로드</a>
				</span>	  
...
// 상품관리 모달창
function upload(){
	$.ajaxLoding('ga_menu3_52_upload.php',$('.layerBody_item'),$('#modal2'));	
}
 ...
 $(document).ready(function(){
	...
	//window.parent.postMessage("계약수납관리 > 계약정보업로드", "*");   // '*' on any domain 부모로 보내기..        
	...
	$(".rowData > td").click(function(){
		...
		if($(trData).find("td").index($(this))!='10'){
			var popOpen	= window.open("<?=$conf['homeDir']?>/sub/menu3/ga_menu3_52_list.php?upldate="+upldate+"&gubun=" +gubun+"&gubunsub=" + gubunsub+"&uplnum=" +uplnum +"&filename=" +filename                    ,"width=1200px,height=950px,top="+top+",left="+left+",status=0,toolbar=0,menubar=0,location=false,scrollbars=yes");
			popOpen.focus();
		} 
	});
