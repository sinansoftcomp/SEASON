...
require_once $_SERVER['DOCUMENT_ROOT']."/bin/include/lib/PHPExcel/Classes/PHPExcel/IOFactory.php";  // IOFactory.php을 불러와야 하며, 경로는 사용자의 설정에 맞게 수정해야 한다.					
	...
	function pattern_ser ($objWorksheet,$mscon,$col_cnt){
				...
				for ($ed=0;  $ed <= $col_cnt  ; $ed++ ) {    //나중에 필드 숫자만큼  하기 
						...
						$tit2 =  preg_replace('/\s+/', '',$tit2);										
	 					...
	 					if ($tit == '증권번호' || $tit == '계약번호'  ) {       //둘중하나가 있으면 그 라인이 타이틀이다  이라인을 통과하면 안됨 (다음이 빈칸이어도 증권번호가 없으면 다음PRG에서 통과한다)
							...
							$ser_tit_yes  = 'Y';	//일단 증권번호 타이틀은 찾았다.
		...
		//======================================================================
		//-->타이틀 완성후 패턴찾기      타이틀을 찾았으면 해당엑셀의 패턴을 찾는다 
		//======================================================================
		$EXCEL_declare = array( // 1차원 배열을 1개 갖는 2차원 배열 선언
			array()
		);
 
		$sql = "select *  from UPLOAD_EXCEL where scode = '".$_SESSION['S_SCODE']."' and gubun = 'P'   order by upcnt desc "; //---비교사항이 만은것 부터
		//$sql = "select *  from UPLOAD_EXCEL where scode = '".$_SESSION['S_SCODE']."' and gubun = 'P'  and gubunsub ='P2' ";
		...
		for ($db_i = 0; $db_i < count($ptData) ;$db_i++ ) {
				$b_Status = $b_Status.'<br>-----------------------------------------------------------' ;
				$b_Status = $b_Status.'<br> <strong>비교패턴 : </strong>'. $db_i.'    :    '. $ptData[$db_i]['CODE'] .' - ' . $ptData[$db_i]['GUBUN'] .'-' . $ptData[$db_i]['GUBUNSUB'] .'-  ' . $ptData[$db_i]['GNAME'] . '  : '  ;
				...
				if ($ybit =='Y') {
					...
					$EXCEL_data_tit['INSILJ']   = $ptData[$db_i]['INSILJ'] ;         //상품구분
