			...
			for ($sheet_no=0 ; $sheet_no < $sheet_tot_cnt ; $sheet_no++) { 
					...
					*/ 
					...
					for ($i = $start_line ; $i <= $maxRow ; $i++) {		//정의된 시작 행부터 부터 데이터시작
							...
							$num = [13, 19, 21, 23, 25, 27, 29, 31, 33, 35,36];
							foreach ($num as $num) {
							...
							//---->한건의data입력 시작 start. 
							$ins_ipmst_excel = "						   
													   '$EXCEL_data[1]','$EXCEL_data[2]','$EXCEL_data[3]','$EXCEL_data[3]','$EXCEL_data[5]','$EXCEL_data[6]','$EXCEL_data[7]','$EXCEL_data[8]','$EXCEL_data[9]','$EXCEL_data[10]',
													   ...
													   '$EXCEL_data[21]','$EXCEL_data[22]','$EXCEL_data[23]','$EXCEL_data[24]', '$EXCEL_data[25]','$EXCEL_data[26]','$EXCEL_data[27]','$EXCEL_data[28]', '$EXCEL_data[29]','$EXCEL_data[30]' , 
														'$EXCEL_data[31]' , '$EXCEL_data[32]' ,'$EXCEL_data[33]' ,'$EXCEL_data[34]','$EXCEL_data[35]','$EXCEL_data[36]'
													  ";
							$sql = "
									insert into ins_ipmst( ".$ins_ipmst." )
									values('".$_SESSION['S_SCODE']."','".$upldate."','".$pt[GUBUN]."','".$pt[GUBUNSUB]."','".$LS_UPLNUM. "','".$i."','".'200412'."',".$ins_ipmst_excel ."
							...
							if ($result == false){
								...
								$exc_bit = 'N' ;
								$e = ' 계약엑셀업로드 중 오류발생  엑셀오류행은  엑셀 '. $i.' 행입니다 ';
								print_r($e );
								print_r($sql );
								break;
